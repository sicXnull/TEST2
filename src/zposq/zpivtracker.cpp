...
#include "zposq/zposqwallet.h"
#include "witness.h"

using namespace std;

CzPOSQTracker::CzPOSQTracker(std::string strWalletFile)
{
...
CzPOSQTracker::~CzPOSQTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzPOSQTracker::Init()
{
...
bool CzPOSQTracker::Archive(CMintMeta& meta)
{
...
bool CzPOSQTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzPOSQTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzPOSQTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzPOSQTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzPOSQTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzPOSQTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzPOSQTracker::GetSerialHashes()
{
...
CAmount CzPOSQTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzPOSQTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzPOSQTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzPOSQTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzPOSQTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzPOSQTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzPOSQTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzPOSQTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzPOSQTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzPOSQTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzPOSQTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzPOSQWallet* zPOSQWallet)
{
    bool iszPOSQWalletInitialized = (NULL != zPOSQWallet);
    CMintMeta meta;
    ...
    if (! iszPOSQWalletInitialized)
        zPOSQWallet = new CzPOSQWallet(strWalletFile);
    meta.isSeedCorrect = zPOSQWallet->CheckSeed(dMint);
    if (! iszPOSQWalletInitialized)
        delete zPOSQWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzPOSQTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPOSQTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzPOSQTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzPOSQTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzPOSQTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzPOSQTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzPOSQWallet* zPOSQWallet = new CzPOSQWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zPOSQWallet);
        }
        delete zPOSQWallet;
        LogPrint("zero", "%s: added %d dzposq from DB\n", __func__, listDeterministicDB.size());
    }
